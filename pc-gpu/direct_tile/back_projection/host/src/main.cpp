...

{
	...
	// User-visible output - Platform information
    {
      ...
      printf("==========================\n");
      clGetPlatformInfo((dev->platforms)[1], CL_PLATFORM_NAME, STRING_BUFFER_LEN, char_buffer, NULL);
      printf("%-40s = %s\n", "CL_PLATFORM_NAME", char_buffer);
      clGetPlatformInfo((dev->platforms)[1], CL_PLATFORM_VENDOR, STRING_BUFFER_LEN, char_buffer, NULL);
      printf("%-40s = %s\n", "CL_PLATFORM_VENDOR ", char_buffer);
      clGetPlatformInfo((dev->platforms)[1], CL_PLATFORM_VERSION, STRING_BUFFER_LEN, char_buffer, NULL);
 ...
 *
 */
void calc_tiles(device_info* devi,sinograms* g,int size,float* tau,float* pix,int divide)
{
  int m;        //x direction
  int n;        //y direction
  float* Sino=NULL;
  ...
  devi->kernel=NULL;
  devi->kernel=clCreateKernel(devi->program,"calculate_tile",&(devi->status));
  ...
  (devi->status)=clSetKernelArg(devi->kernel,12,sizeof(int),&divide);
...
int main(int argc,char* argv[])
{
  ...
  char* filename2 = OUTPUT_FILENAME;
	...
	//argument4 is the divide size
	if(argv[4]==NULL){
		printf("Divide size not set\n");
		exit(0);
	}
	divideN= atoi(argv[4]);
    ...
    calc_tiles(dev,sino,size,tau,pixel,divideN);
