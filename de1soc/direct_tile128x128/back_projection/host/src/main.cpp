...

{
	...
	dev->program = createProgramFromBinary(dev->context,"kernels_tile.aocx",&((dev->devices)[0]),1);
 ...
 *
 */
void calc_tiles(device_info* devi,sinograms* g,int size,float* tau,float* pix)
{
  int m;        //x direction
  int n;        //y direction
  float* Sino=NULL;
...
int main(int argc,char* argv[])
{
  ...
  char* filename2 = OUTPUT_FILENAME;
	...
	//argument4 is the divide size - fixed and equals 2
	//if(argv[4]==NULL){
	...
	calc_tiles(dev, sino, size, tau, pixel);
